...
/**
 * 获取 Hermes Agent 的主目录路径
 * @returns {string} Hermes 主目录路径
 */
function hermesHome() {
...
/**
 * 构建增强的 PATH 环境变量，包含 Hermes 和 uv 相关路径
 * @returns {string} 增强后的 PATH 环境变量
 */
...
/**
 * 从 Hermes 配置文件中读取网关端口号
 * @returns {number} 网关端口号，默认 8642
 */
...
/**
 * 获取 Hermes 网关的完整 URL 地址
 * @returns {string} 网关 URL 地址
 */
...
/**
 * 静默执行 Hermes 相关命令（不显示输出）
 * @param {string} program - 要执行的程序名
 * @param {string[]} args - 程序参数数组
 * @returns {Object} 执行结果 {ok: boolean, stdout?: string, stderr?: string}
 */
...
/**
 * 规范化用户自定义的 OpenClaw 目录路径
 * @param {string} raw - 原始路径字符串
 * @returns {string|null} 规范化后的绝对路径，无效输入返回 null
 */
...
/**
 * 应用 OpenClaw 路径配置，更新全局路径变量
 * @param {Object} panelConfig - 面板配置对象
 * @returns {Object} {path: string, isCustom: boolean} 返回路径信息和是否为自定义路径
 */
...
/**
 * 获取 CLI 路径的规范真实路径（解析符号链接）
 * @param {string} raw - 原始路径字符串
 * @returns {string|null} 规范化的真实路径，失败时返回普通规范化路径
 */
...
/**
 * 扫描 CLI 工具的身份标识路径（在 Windows 上优先使用 .cmd 包装器）
 * @param {string} rawPath - 原始路径
 * @returns {string} 身份标识路径
 */
...
/**
 * 检查 CLI 路径是否为被拒绝的路径（如 CherryStudio 相关路径）
 * @param {string} cliPath - CLI 路径
 * @returns {boolean} 是否为被拒绝的路径
 */
...
/**
 * 添加 CLI 候选路径到列表中（去重并验证）
 * @param {string[]} candidates - 候选路径数组
 * @param {Set} seen - 已见过的路径集合
 * @param {string} rawPath - 原始路径
 */
...
/**
 * 查找命令的完整路径（使用 which 或 where 命令）
 * @param {string} command - 命令名称
 * @returns {string|null} 命令的完整路径，未找到返回 null
 */
 ...
 * 规范化命令路径（区分路径和普通命令名）
 * @param {string} raw - 原始路径字符串
 * @returns {string|null} 规范化后的路径或命令名
 */
...
/**
 * 读取配置中指定的 Git 路径
 * @returns {string|null} 配置的 Git 路径
 */
...
/**
 * 解析 Git 可执行文件路径（优先使用配置的路径）
 * @returns {Object} {gitPath: string, isCustom: boolean, isPathLike: boolean}
 */
 ...
 * @param {Object} resolved - 解析后的 Git 可执行文件信息
 ...
 * @returns {Object} {gitPath, isCustom, isPathLike, result}
 */
...
/**
 * 读取配置中指定的 OpenClaw 搜索路径列表
 * @returns {string[]} 规范化的搜索路径数组
 */
...
/**
 * 检测 Windows 上的 npm shim 文件来源
 * @param {string} cliPath - CLI 路径
 * @returns {string|null} shim 来源类型 ('npm-zh' | 'npm-official' | null)
 */
 ...
 * 分类 CLI 工具的来源类型
 * @param {string} cliPath - CLI 路径
 * @returns {string} 来源类型 ('standalone' | 'npm-zh' | 'npm-official' | 'npm-global' | 'unknown')
 */
...
/**
 * 规范化 CLI 安装来源为内部标识
 * @param {string} cliSource - 原始来源类型
 * @returns {string} 规范化后的来源 ('chinese' | 'official' | 'unknown')
 */
...
/**
 * 从安装目录中读取 OpenClaw 版本号
 * @param {string} cliPath - CLI 工具路径
 * @returns {string|null} 版本号，未找到返回 null
 */
...
/**
 * 读取 where/which 命令输出的所有 OpenClaw 候选路径
...
/**
 * 读取 Windows 上的 npm 全局安装前缀路径
 * @returns {string|null} npm 全局前缀路径
 */
...
/**
 * 获取指定来源的推荐 OpenClaw 版本
 * @param {string} source - 来源类型 ('chinese' | 'official')
 * @returns {string|null} 推荐版本号
 */
...
/**
 * 配置 Git 的 HTTPS 重写规则（将 SSH/Git 协议重写为 HTTPS）
 * @returns {number} 成功配置的规则数量
 */
...
/**
 * 构建 Git 安装所需的环境变量（禁用交互式提示、配置 SSH）
...
/**
 * 解析 Skill 的 frontmatter 元数据（从 .md 文件中）
 * @param {string} skillMdPath - Skill 的 markdown 文件路径
...
/**
 * 解析 Agent 的 Skills 目录路径
 * @param {string} agentId - Agent ID
 * @returns {string|null} Skills 目录路径
 */
 ...
 * 检查密码强度
 * @param {string} pw - 密码
 * @returns {string|null} 错误消息，通过返回 null
 */
 ...
 * @param {string|null} endpoint - Docker endpoint
...
const handlers = {
  ...
  remove_messaging_platform({ platform, accountId }) {
    ...
    if (normalizedAccountId) {
      if (cfg.channels?.[storageKey]?.accounts && typeof cfg.channels[storageKey].accounts === 'object') {
        ...
        // 若账号全部删完，移除整个平台条目
        if (Object.keys(cfg.channels[storageKey].accounts).length === 0) {
  ...
  auto_install_node() {
    // Web 模式下不自动安装系统软件，返回指引
    throw new Error('Web 部署模式下请手动安装 Node.js：\n请访问 https://nodejs.org 下载安装，或使用包管理器：\n- Ubuntu/Debian: sudo apt install nodejs npm\n- CentOS/RHEL: sudo yum install nodejs npm\n- macOS: brew install node')
  },
