...
#[tauri::command]
pub fn panel_log_command(message: String) -> Result<(), String> {
    crate::commands::panel_log(&message);
    Ok(())
}
...
fn merge_configs_preserving_fields(existing: &Value, new: &Value) -> Value {
    ...
    match (existing, new) {
        (Value::Object(existing_obj), Value::Object(new_obj)) => {
            ...
            // 先移除 new 中不存在的需要完整替换的节点
            for &key in FULL_REPLACE_KEYS {
            ...
            for (key, new_value) in new_obj {
                if let Some(existing_value) = existing_obj.get(key) {
                    // 需要完整替换的节点：直接使用新值
                    if FULL_REPLACE_KEYS.contains(&key.as_str()) {
...
pub async fn get_version_info() -> Result<VersionInfo, String> {
    // Phase 1: 三重并行 —— 本地版本检测、CLI 路径解析、安装来源检测同时启动
    // get_local_version 中的 openclaw status 等待期间，其余两项可同时完成
    ...
    // Phase 2: 并行执行网络请求（get_latest_version_for）和安装扫描（spawn_blocking）
    let cli_path_for_scan = cli_path.clone();
    let (latest, all_installations) = futures_util::join!(
        ...
        tokio::task::spawn_blocking(move || scan_all_installations(&cli_path_for_scan)),
    );
...
#[tauri::command]
pub fn scan_node_paths(app: tauri::AppHandle) -> Result<Value, String> {
    use tauri::Emitter;
    let _ = app.emit("fix-log", "🔍 Scanning Node.js paths...");
...
/// 运行 openclaw doctor --fix 自动修复配置问题（流式输出）
#[tauri::command]
pub async fn doctor_fix(app: tauri::AppHandle) -> Result<Value, String> {
    use crate::utils::openclaw_command_async;
    use tauri::Emitter;
    use tokio::io::{AsyncBufReadExt, BufReader};

    let mut child = openclaw_command_async()
        .args(["doctor", "--fix"])
        .stdout(std::process::Stdio::piped())
        ...
        .map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                "OpenClaw CLI 未找到，请先安装".to_string()
            } else {
                format!("执行 doctor 失败: {e}")
            }
        })?;
    ...
    Ok(json!({
        "success": success,
        "output": all_stdout.trim(),
        "errors": all_stderr.trim(),
...
async fn offline_install_node_msi(app: &tauri::AppHandle) -> Result<String, String> {
    ...
    // ----- 版本和下载地址（静态版本，可定期更新或改为动态获取）-----
    let version_str = "24.0.0".to_string(); // 可改为从镜像 index 动态获取最新 LTS
    let msi_filename = format!("node-v{}-x64.msi", version_str);
    ...
    // ---------- 2. 创建临时目录 ----------
    let temp_dir = std::env::temp_dir().join("openclaw-node-install");
    if temp_dir.exists() {
    ...
    let file_exists = {
        ...
        // 强制刷盘并释放文件句柄，确保 msiexec 能访问
        file.sync_all().map_err(|e| format!("刷盘失败: {e}"))?;
        drop(file);
        true
    };

    let _ = app.emit("upgrade-log", "✅ MSI 文件下载完成");
    ...
    if total_size > 0 && actual_size != total_size {
        let _ = std::fs::remove_dir_all(&temp_dir);
        return Err(format!(
            "下载不完整：预期 {} 字节，实际 {} 字节。请重试或手动安装: {}",
            total_size, actual_size, download_url
        ));
    }
    ...
    // ---------- 5. 开始安装（多级递进策略）----------
    let msi_str = msi_path_clone
        .to_str()
        .ok_or_else(|| "MSI 路径包含非 UTF-8 字符".to_string())?;

    // 安装前确认文件存在并输出路径（便于排查 1619 错误）
    if !msi_path_clone.exists() {
        return Err(format!("MSI 文件不存在，可能被系统清理: {}", msi_str));
    }
    let _ = app.emit("upgrade-log", format!("📦 MSI 文件路径: {}", msi_str));
    let _ = app.emit("upgrade-log", "🔧 开始安装 Node.js...");

    // --- 5.1 尝试完全静默安装 (/qn) + 重试 ---
    let mut success = false;
    for attempt in 0..2 {
        if attempt > 0 {
            let _ = app.emit("upgrade-log", "🔄 静默安装失败，等待 3 秒后重试...");
    ...
    if success {
        let _ = std::fs::remove_dir_all(&temp_dir);
        let _ = app.emit("upgrade-log", "✅ Node.js 静默安装成功！重启应用后生效。");
        return Ok("Node.js 已成功安装，请重启应用".to_string());
    }

    // --- 5.2 尝试 /quiet 模式（与 /qn 类似但兼容性更好） ---
    ...
    match msiexec_status {
        Ok(s) if s.success() => {
            ...
            Ok("Node.js 已成功安装，请重启应用".to_string())
        }
        Ok(s) => {
            let code = s.code().unwrap_or(-1);
            let diag = msi_exit_code_diag(code);
...
fn msi_exit_code_diag(code: i32) -> &'static str {
    match code {
        0 => "成功",
        13 => "数据无效",
        87 => "参数无效",
...
pub async fn auto_install_node(app: tauri::AppHandle) -> Result<String, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match winget_result {
            Ok(mut child) => {
                ...
                if status.success() {
                    let _ = app.emit("upgrade-log", "Node.js 安装成功！重启应用后生效。");
                    return Ok("Node.js 已通过 winget 安装，请重启应用".to_string());
                }
                // winget 安装失败，继续尝试离线安装
            }
            Err(e) => {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if Command::new("brew")
            ...
            .unwrap_or(false)
        {
            let _ = app.emit("upgrade-log", "尝试使用 brew 安装 node...");
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if status.success() {
            let _ = app.emit("upgrade-log", "Node.js 安装成功！重启应用后生效。");
            return Ok("Node.js 已安装，请重启应用".to_string());
        }
        return Err("Node.js 安装失败，请手动安装: https://nodejs.org".to_string());
    }

    #[allow(unreachable_code)]
    Err("当前平台不支持自动安装 Node.js，请手动安装: https://nodejs.org".to_string())
}

// ========================================
// 同步封装函数（供 lib.rs 首次启动检测使用）
// ========================================

/// 检查 Node.js 是否已安装（同步版本）
#[allow(dead_code)]
