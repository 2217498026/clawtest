...
/// 下载 RAR 压缩包并解压到目标目录
/// url: RAR 文件的下载 URL
/// target_dir: 解压目标目录（如 ~/.openclaw/skills）
#[tauri::command]
pub async fn download_and_extract_rar(url: String, target_dir: String) -> Result<Value, String> {
    ...
    eprintln!("[download_and_extract_rar] 开始下载: {}", &url);
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| format!("下载 RAR 包失败: {e}"))?
        .error_for_status()
        .map_err(|e| format!("下载 RAR 包 HTTP 错误: {e}"))?;
    ...
    let mut archive = unrar::Archive::new(tmp_path.to_string_lossy().as_ref())
        .as_first_part()
        .open_for_processing()
        .map_err(|e| format!("打开 RAR 文件失败: {e}"))?;

    loop {
        match archive.read_header() {
            Ok(Some(next)) => {
                let filename = next.entry().filename.to_string_lossy().to_string();
                eprintln!("  提取: {}", filename);
                archive = next
                    .extract_with_base(&target_dir)
                    .map_err(|e| format!("提取文件失败 ({}): {e}", filename))?;
            }
            Ok(None) => break,
            Err(e) => {
                return Err(format!("读取 RAR 归档条目失败: {e}"));
            }
        }
    }
    ...
    eprintln!("[download_and_extract_rar] 完成");
    Ok(serde_json::json!({
        "success": true,
        "target": target_dir,
    }))
