...
pub(crate) fn panel_log(message: &str) {
    let log_dir = openclaw_dir().join("clawpanel").join("logs");
    let _ = std::fs::create_dir_all(&log_dir);
    let path = log_dir.join("panel.log");
    let line = format!(
        "[{}] {}\n",
...
fn gateway_listen_port_inner() -> u16 {
    panel_log("[gateway_port] 开始获取 Gateway 监听端口");

    panel_log("[gateway_port] 尝试获取缓存锁 (GATEWAY_PORT_CACHE)");
    let cache_lock_result = GATEWAY_PORT_CACHE.lock();
    match cache_lock_result {
        Ok(cache) => {
            let elapsed = cache.1.elapsed();
            panel_log(&format!(
                "[gateway_port] 缓存锁获取成功, 缓存端口={}, 缓存年龄={:?}",
                cache.0, elapsed
            ));
    ...
    panel_log("[gateway_port] 开始从配置文件读取端口");
    let port = read_gateway_port_from_config();
    panel_log(&format!("[gateway_port] 从配置文件读取到端口: {}", port));

    panel_log("[gateway_port] 尝试更新缓存");
    match GATEWAY_PORT_CACHE.lock() {
        Ok(mut cache) => {
