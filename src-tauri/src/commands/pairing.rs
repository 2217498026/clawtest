...
#[tauri::command]
pub fn auto_pair_device(app: tauri::AppHandle) -> Result<String, String> {
    use tauri::Emitter;
    let _ = app.emit("fix-log", "🔗 Starting device pairing...");
    // 无论是否已配对，都确保 gateway.controlUi.allowedOrigins 已写入
    // 必须在最前面，避免因设备密钥不存在而跳过
    patch_gateway_origins();
    let _ = app.emit("fix-log", "✅ Gateway origins patched");
    ...
    if let Some(existing) = paired.get_mut(&device_id) {
        ...
        let _ = app.emit("fix-log", "✅ Device already paired");
    ...
    let _ = app.emit("fix-log", "✅ Device paired successfully");
    Ok("设备配对成功".into())
}
