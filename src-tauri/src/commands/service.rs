...
async fn guardian_tick(app: &tauri::AppHandle) {
    let snapshot = match gateway_service_status().await {
        Ok(Some(svc)) => svc,
        Ok(None) => {
            guardian_log("guardian_tick: 未找到 Gateway 服务，跳过本轮检查");
            return;
        }
        Err(err) => {
    ...
    let (restart_attempt, emit_give_up) = {
        ...
        if state.last_seen_running.is_none() {
            guardian_log(&format!(
                "guardian_tick: 首次 tick，初始化状态 running={running} ready={ready}"
            ));
        ...
        if !ready {
            guardian_log(&format!(
                "guardian_tick: Gateway 未就绪 (cli_installed={} config_exists={})，跳过守护",
        ...
        if state.pause_reason.is_some() {
            guardian_log(&format!(
                "guardian_tick: 守护已暂停 (原因: {})，跳过本轮",
                state.pause_reason.as_ref().unwrap()
            ));
            state.last_seen_running = Some(running);
        ...
        if !was_running {
            guardian_log("guardian_tick: Gateway 持续停止中（非异常退出），不触发重启");
            return;
        }

        if state.manual_hold {
            guardian_log("guardian_tick: 处于手动停机保持状态，不触发重启");
            return;
        }

        if state.give_up {
            guardian_log("guardian_tick: 已放弃自动拉起，不触发重启");
            return;
        }
        ...
        if let Some(last) = state.last_restart_time {
            if now.duration_since(last) < GUARDIAN_RESTART_COOLDOWN {
                guardian_log(&format!(
                    "guardian_tick: 距上次重启不足 {}s，冷却中，跳过",
                    GUARDIAN_RESTART_COOLDOWN.as_secs()
                ));
                return;
            }
        }
...
pub fn start_backend_guardian(app: tauri::AppHandle) {
    guardian_log("start_backend_guardian 被调用，尝试启动后端守护");

    if GUARDIAN_STARTED.swap(true, Ordering::SeqCst) {
        guardian_log("后端守护已在运行，跳过重复启动");
        return;
    }

    guardian_log("GUARDIAN_STARTED 标记已设置，开始初始化");

    #[cfg(target_os = "windows")]
    {
        guardian_log("Windows 平台：将 cleanup_zombie 提交到异步任务（避免阻塞 setup）");
        tauri::async_runtime::spawn(async move {
            ...
            match result {
                ...
                Ok(Err(join_err)) => {
                    let msg = format!("cleanup_zombie spawn_blocking join error: {}", join_err);
                    guardian_log(&msg);
                }
                Err(_timeout) => {
                    guardian_log("cleanup_zombie 执行超时 (30s)，已放弃等待");
                }
            }
        });
    }

    guardian_log("后端守护循环已启动，spawn 异步任务");
    tauri::async_runtime::spawn(async move {
        guardian_log("后端守护异步任务已进入主循环");
        loop {
            guardian_tick(&app).await;
            tokio::time::sleep(GUARDIAN_INTERVAL).await;
        }
    });
    guardian_log("后端守护异步任务已提交到运行时");
}
...
mod platform {
    ...
    fn get_last_known_pid() -> Option<u32> {
        match LAST_KNOWN_GATEWAY_PID.lock() {
            Ok(guard) => *guard,
            Err(e) => {
                super::guardian_log(&format!("get_last_known_pid: 锁中毒: {:?}", e));
                None
            }
        }
    }

    fn set_last_known_pid(pid: Option<u32>) {
        match LAST_KNOWN_GATEWAY_PID.lock() {
            Ok(mut guard) => *guard = pid,
            Err(e) => {
                super::guardian_log(&format!("set_last_known_pid: 锁中毒: {:?}", e));
            }
        }
    }

    fn get_active_child_pid() -> Option<u32> {
        match ACTIVE_GATEWAY_CHILD.lock() {
            Ok(guard) => *guard,
            Err(e) => {
                super::guardian_log(&format!("get_active_child_pid: 锁中毒: {:?}", e));
                None
            }
        }
    }
    ...
    fn set_active_child_pid(pid: Option<u32>) {
        match ACTIVE_GATEWAY_CHILD.lock() {
            Ok(mut guard) => *guard = pid,
            Err(e) => {
                super::guardian_log(&format!("set_active_child_pid: 锁中毒: {:?}", e));
            }
        }
    }
    ...
    /// 清理残留的 Gateway lock 文件（%TEMP%\openclaw\gateway.*.lock）
    ...
    fn cleanup_gateway_lock_files() {
        ...
        let openclaw_dir = temp.join("openclaw");
        let lock_dir = openclaw_dir.join("gateway");
        super::guardian_log(&format!(
            "cleanup_gateway_lock_files: 检查目录 {:?}",
            lock_dir
        ));

        if !lock_dir.exists() {
            super::guardian_log("cleanup_gateway_lock_files: lock 目录不存在，跳过");
            return;
        }
        ...
        let entries = match fs::read_dir(&lock_dir) {
            ...
            Err(e) => {
                super::guardian_log(&format!(
                    "cleanup_gateway_lock_files: 无法读取目录 {:?}: {e}",
                    lock_dir
                ));
                return;
            }
        ...
        for entry in entries.flatten() {
            ...
            if filename.ends_with(".lock") && filename.contains("gateway") {
                match fs::remove_file(&path) {
                    Ok(()) => {
                        removed += 1;
                        super::guardian_log(&format!(
                            "cleanup_gateway_lock_files: 已删除 lock 文件 {:?}",
                            path
                        ));
                    }
                    Err(e) => {
                        super::guardian_log(&format!(
                            "cleanup_gateway_lock_files: 无法删除 {:?}: {e}",
                            path
                        ));
                    }
                }
        ...
        super::guardian_log(&format!(
            "cleanup_gateway_lock_files: 共清理 {removed} 个 gateway lock 文件"
        ));
    }

    /// 清理残留的 Windows Scheduled Task "OpenClaw Gateway"
        ...
        match output {
            Ok(out) => {
                if out.status.success() {
                    ...
                    super::guardian_log(&format!(
                        "cleanup_gateway_scheduled_task: 删除失败（任务可能不存在）: {stderr}"
                    ));
                }
            }
            Err(e) => {
                super::guardian_log(&format!(
                    "cleanup_gateway_scheduled_task: schtasks 执行失败: {e}"
                ));
            }
        }
    }
    ...
    pub(crate) fn cleanup_zombie_gateway_processes() {
        super::guardian_log("cleanup_zombie_gateway_processes 开始执行");
        ...
        for pid in &pids {
            ...
            if let Some(cmdline) = read_process_command_line(pid) {
                ...
                let our_pid = get_last_known_pid();

                super::guardian_log(&format!(
                    "cleanup_zombie: PID {pid} is_gateway={is_gateway} our_pid={our_pid:?}"
                ));
                ...
                if is_gateway {
                    if !responsive {
                        ...
                        cleanup_gateway_lock_files();
                        cleanup_gateway_scheduled_task();
                    } else if Some(pid) != our_pid {
                        super::guardian_log(&format!(
                            "检测到外部启动的 Gateway 进程 (PID {pid})：/health 正常响应，已采纳"
                        ));
                        set_last_known_pid(Some(pid));
                    } else {
                        super::guardian_log(&format!(
                            "cleanup_zombie: PID {pid} 为当前实例的已知进程，跳过"
                        ));
                    }
                } else {
                    super::guardian_log(&format!(
                        "cleanup_zombie: PID {pid} 非 Gateway 进程，跳过"
                    ));
                }
            } else {
                super::guardian_log(&format!(
                    "cleanup_zombie: PID {pid} 无法读取命令行，跳过（避免误杀）"
                ));
            }
        }

        super::guardian_log("cleanup_zombie_gateway_processes 执行完毕");
    }
    ...
    pub async fn start_service_impl(_label: &str) -> Result<(), String> {
        ...
        // 清理残留的 lock 文件和 Scheduled Task，防止 openclaw gateway start 拒绝启动
        cleanup_gateway_lock_files();
        cleanup_gateway_scheduled_task();
       ...
       /*  cmd.arg("gateway")
        ...
        set_active_child_pid(Some(spawned_pid));
    ...
    pub async fn stop_service_impl(_label: &str) -> Result<(), String> {
        ...
        let pids_to_kill: Vec<u32> = {
            let active = get_active_child_pid();
            let known = get_last_known_pid();
        ...
        // 清理残留的 lock 文件和 Scheduled Task，防止下次 start 失败
        cleanup_gateway_lock_files();
        cleanup_gateway_scheduled_task();
            ...
            Ok(Ok(status)) => {
                if !status.success() {
                    ...
                    if action_owned == "restart" {
                        super::guardian_log("[gateway_command] restart 失败，尝试强制清理后重启");
                        cleanup_zombie_gateway_processes();
                        tokio::time::sleep(Duration::from_millis(500)).await;
...
pub async fn restart_service(app: tauri::AppHandle, label: String) -> Result<(), String> {
    use tauri::Emitter;
    let _ = app.emit("fix-log", format!("⚡ Restarting service: {}", label));
    let (running, pid) = current_gateway_runtime(&label).await;
    if running {
        ensure_owned_gateway_or_err(pid)?;
        let _ = app.emit("fix-log", "✅ Ownership verified");
    }
    ...
    if result.is_ok() {
        let _ = app.emit("fix-log", "✅ Service restarted");
    }
    result
}
