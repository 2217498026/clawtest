...
export default {
  ...
  desc: _('安装和配置 智能体', 'Install and configure 智能体', '安裝和設定 智能体', '智能体 のインストールと設定', '智能体 설치 및 설정', 'Cài đặt và cấu hình 智能体', 'Instalar y configurar 智能体', 'Instalar e configurar 智能体', 'Установка и настройка 智能体', 'Installer et configurer 智能体', '智能体 installieren und konfigurieren'),
  ...
  headerDesc: _('智能体 AI Agent 框架的桌面管理面板', 'Desktop management panel for 智能体 AI Agent framework', '', '智能体 AI Agent フレームワークのデスクトップ管理パネル', '智能体 AI Agent 프레임워크의 데스크톱 관리 패널'),
  ...
  stepNodeHint: _('智能体 基于 Node.js 运行，请先安装。', '智能体 requires Node.js, please install it first.', '智能体 基於 Node.js 執行，請先安裝。', '智能体 は Node.js で動作します。先にインストールしてください。'),
  ...
  cliAheadWarning: _('检测到当前本地 智能体 {current} 高于当前面板推荐稳定版 {recommended}，可能存在兼容或稳定性风险。建议稍后到「关于」页回退到推荐版。', 'Detected local 智能体 {current} is ahead of the panel\'s recommended stable version {recommended}. There may be compatibility or stability risks. Consider rolling back to the recommended version on the About page.', '檢測到目前本地 智能体 {current} 高於目前面板推薦穩定版 {recommended}，可能存在相容或穩定性風險。建議稍后到「關於」頁回退到推薦版。'),
  ...
  customDirTitle: _('自定义 智能体 配置目录', 'Custom 智能体 Config Directory', '自定義 智能体 設定目錄', 'カスタム 智能体 設定ディレクトリ', '사용자 정의 智能体 설정 디렉터리'),
  ...
  customDirNotice: _('这里只影响配置文件与状态目录，不会替代上方选择的 智能体 CLI 可执行文件。', 'This only changes the config/state directory and does not replace the 智能体 CLI executable selected above.', '這裡只會影響設定檔與狀態目錄，不會取代上方選擇的 智能体 CLI 可執行檔。', 'ここで変更されるのは設定/状態ディレクトリのみで、上で選択した 智能体 CLI 実行ファイルは置き換えられません。'),
  ...
  envHintTitle: _('找不到已安装的 智能体？', 'Can\'t find installed 智能体?', '找不到已安裝的 智能体？'),
  envHintDesc: _('ClawPanel 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 智能体。若你的 智能体 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', 'ClawPanel desktop can only directly manage 智能体 installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your 智能体 actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', 'ClawPanel 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 智能体。若你的 智能体 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  envHintWsl: _('WSL (Windows 子系统)', 'WSL (Windows Subsystem for Linux)', 'WSL (Windows 子系統)'),
  envHintWslDesc: _('智能体 装在 WSL 里，Windows 侧无法访问', '智能体 in WSL is not accessible from Windows side', '智能体 裝在 WSL 里，Windows 側無法訪問'),
  ...
  envHintLocalReinstall: _('如果你要管理的是当前这台电脑里的 智能体，更建议直接使用本页的「安装 智能体」卡片，或在上方手动绑定 CLI 路径。', 'If you want to manage 智能体 on this computer, use the "Install 智能体" card on this page or bind the CLI path above.', '如果你要管理的是目前這台電腦裡的 智能体，更建議直接使用本頁的「安裝 智能体」卡片，或在上方手動綁定 CLI 路徑。'),
  searchOpenclawTitle: _('本机已经装了 智能体？', 'Already installed 智能体 locally?', '本機已經安裝 智能体？'),
  searchOpenclawDesc: _('可以主动搜索本机常见安装位置，并从结果中选择当前要管理的 智能体 CLI。', 'You can actively search common local install locations and choose which 智能体 CLI this panel should manage.', '可以主動搜尋本機常見安裝位置，並從結果中選擇目前要管理的 智能体 CLI。'),
  searchOpenclawBtn: _('搜索 智能体', 'Search 智能体', '搜尋 智能体'),
  ...
  searchOpenclawExtraPathsPlaceholder: _('每行一个 CLI 文件或安装目录，例如 D:\\Tools\\智能体 或 E:\\AI\\openclaw.cmd', 'One CLI file or install directory per line, e.g. D:\\Tools\\智能体 or E:\\AI\\openclaw.cmd', '每行一個 CLI 檔案或安裝目錄，例如 D:\\Tools\\智能体 或 E:\\AI\\openclaw.cmd'),
  ...
  searchOpenclawHint: _('搜索结果里点击「使用」即可绑定当前 CLI；若随后仍提示配置文件缺失，请到右侧配置卡里填写自定义 智能体 配置目录。', 'Click "Use" on a result to bind that CLI. If config is still missing afterwards, set the custom 智能体 config directory in the config card on the right.', '在搜尋結果中點擊「使用」即可綁定目前 CLI；若之後仍提示設定檔缺失，請到右側設定卡中填寫自定義 智能体 設定目錄。'),
  searchOpenclawScanning: _('正在搜索常见安装位置...', 'Searching common install locations...', '正在搜尋常見安裝位置...'),
  searchOpenclawEmpty: _('没有在常见路径中找到 智能体。你可以继续手动填写 CLI 路径，或直接使用本页的「安装 智能体」卡片。', 'No 智能体 installation was found in common paths. You can keep entering the CLI path manually, or use the "Install 智能体" card on this page.', '沒有在常見路徑中找到 智能体。你可以繼續手動填寫 CLI 路徑，或直接使用本頁的「安裝 智能体」卡片。'),
  searchOpenclawUsing: _('使用中...', 'Using...', '使用中...'),
  searchOpenclawSelectSuccess: _('已选择该 智能体 CLI，正在重新检测...', 'Selected this 智能体 CLI, re-detecting...', '已選擇此 智能体 CLI，正在重新檢測...'),
  searchOpenclawSelectFailed: _('选择失败: {err}', 'Selection failed: {err}', '選擇失敗: {err}'),
  searchOpenclawManualLabel: _('或者手动指定已有 智能体 CLI / 安装目录', 'Or enter an existing 智能体 CLI / install directory manually', '或者手動指定既有 智能体 CLI / 安裝目錄', 'または既存の 智能体 CLI / インストールディレクトリを手動指定'),
  ...
  searchOpenclawManualSaved: _('已绑定该 智能体 CLI，正在重新检测...', 'Bound this 智能体 CLI, re-detecting...', '已綁定此 智能体 CLI，正在重新檢測...', 'この 智能体 CLI を関連付けました。再検出しています...'),
  searchOpenclawManualNotFound: _('在这个位置没有找到可用的 智能体 CLI，请确认路径正确。', 'No usable 智能体 CLI was found at this location. Please verify the path.', '在這個位置沒有找到可用的 智能体 CLI，請確認路徑正確。', 'この場所に利用可能な 智能体 CLI が見つかりませんでした。パスを確認してください。'),
  wslWebHint: _('WSL 中使用 Web 版：', 'Use Web version in WSL:'),
  wslWebDesc: _('如果你的 智能体 本来就运行在 WSL 里，可打开 WSL 终端部署 ClawPanel Web 版：', 'If your 智能体 already runs inside WSL, open a WSL terminal and deploy ClawPanel Web there:', '如果你的 智能体 本來就運行在 WSL 裡，可開啟 WSL 終端部署 ClawPanel Web 版：'),
  ...
  dockerDesc: _('如果你的 智能体 本来就运行在容器里，可在容器内安装 智能体 + ClawPanel Web 版：', 'If your 智能体 already runs inside a container, install 智能体 + ClawPanel Web there:', '如果你的 智能体 本來就運行在容器裡，可在容器內安裝 智能体 + ClawPanel Web 版：'),
  remoteHint: _('远程服务器：', 'Remote server:', '遠程伺服器：'),
  remoteDesc: _('如果 智能体 装在远程服务器上，请 SSH 登录服务器后执行：', 'If 智能体 is installed on a remote server, SSH into that server and run:', '如果 智能体 裝在遠程伺服器上，請 SSH 登入伺服器後執行：'),
  ...
  promptCliMissing: _('智能体 CLI 未安装', '智能体 CLI not installed', '智能体 CLI 未安裝'),
  promptCliOk: _('智能体 CLI 已安装', '智能体 CLI installed', '智能体 CLI 已安裝'),
  promptConfigMissing: _('配置文件不存在', 'Config file not found', '設定檔案不存在'),
  promptConfigOk: _('配置文件正常: {path}', 'Config file OK: {path}', '設定檔案正常: {path}'),
  promptIntro: _('我在安装 智能体 时遇到问题，以下是当前检测状态：', 'I\'m having issues installing 智能体. Here is the current detection status:', '我在安裝 智能体 時遇到問題，以下是目前檢測狀態：'),
  ...
  installOpenclaw: _('安装 智能体', 'Install 智能体', '安裝 智能体', '智能体 をインストール', '智能体 설치', 'Cài đặt 智能体', 'Instalar 智能体', 'Instalar 智能体', 'Установить 智能体', 'Installer 智能体', '智能体 installieren'),
  ...
  installSuccess: _('智能体 安装成功', '智能体 installed successfully', '智能体 安裝成功', '智能体 インストール成功', '智能体 설치 성공'),
  ...
  installScene: _('初始安装 智能体', 'Initial 智能体 installation', '初始安裝 智能体', '智能体 初期インストール'),
}
