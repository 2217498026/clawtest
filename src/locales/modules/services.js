...
export default {
  ...
  desc: _('管理 智能体 服务、检查更新、配置备份', 'Manage 智能体 services, check updates, config backups', '管理 智能体 服務、檢查更新、設定備份', '智能体 サービスの起動・停止・監視', '智能体 서비스 시작·중지·모니터링', 'Khởi động, dừng và giám sát dịch vụ 智能体', 'Iniciar, detener y monitorear servicios 智能体', 'Iniciar, parar e monitorar serviços 智能体', 'Запуск, остановка и мониторинг сервисов 智能体', 'Démarrer, arrêter et surveiller les services 智能体', '智能体-Dienste starten, stoppen und überwachen'),
  ...
  cliNotInstalled: _('智能体 CLI 未安装', '智能体 CLI not installed', '智能体 CLI 未安裝'),
  installCliHint: _('请先安装 智能体 CLI:', 'Please install 智能体 CLI first:', '請先安裝 智能体 CLI:'),
  ...
  foreignGatewayHint: _('检测到外部 Gateway，建议先查看引导，再决定是否切换绑定 CLI 或 智能体 安装路径。', 'External Gateway detected. Review the guidance before deciding whether to switch the bound CLI or 智能体 install path.', '檢測到外部 Gateway，建議先查看引導，再決定是否切換綁定 CLI 或 智能体 安裝路徑。'),
  ...
  guidanceTitleMultiInstall: _('检测到多个 智能体 安装', 'Multiple 智能体 installations detected', '檢測到多個 智能体 安裝'),
  ...
  guidanceSummaryForeignUnbound: _('当前端口上已经有 Gateway 在运行，而你还没有显式绑定 CLI。为了避免误接管，面板会先保持只读查看状态；建议先绑定你要管理的 智能体 实例。', 'A Gateway is already running on the current port and you have not explicitly bound a CLI yet. To avoid taking over the wrong instance, the panel will stay read-only for now. Bind the 智能体 instance you want to manage first.', '目前連接埠上已有 Gateway 在運行，而你尚未明確綁定 CLI。為避免誤接管，面板會先維持唯讀查看狀態；建議先綁定你要管理的 智能体 實例。'),
  guidanceSummaryMultiInstall: _('当前机器检测到多个 智能体 安装。为了避免面板连到错误实例，建议先确认 CLI 绑定与 智能体 安装路径。', 'Multiple 智能体 installations were detected on this machine. To avoid connecting to the wrong instance, confirm the CLI binding and 智能体 install path first.', '目前機器檢測到多個 智能体 安裝。為避免面板連到錯誤實例，建議先確認 CLI 綁定與 智能体 安裝路徑。'),
  guidanceSummaryCheck: _('建议检查当前面板绑定的 CLI 与 智能体 安装路径，确认当前实例与 Gateway 归属一致。', 'Check the CLI and 智能体 install path currently bound to the panel, and confirm they match the intended Gateway instance.', '建議檢查目前面板綁定的 CLI 與 智能体 安裝路徑，確認目前實例與 Gateway 歸屬一致。'),
  ...
  guidanceSuggestionBindAutoDetected: _('1. 你目前还没有在「{settings}」里显式绑定 CLI。建议先去绑定要管理的 智能体 实例，再执行启动、重启或停止。', '1. You have not explicitly bound a CLI in "{settings}" yet. Bind the 智能体 instance you want to manage before trying start, restart, or stop.', '1. 你目前尚未在「{settings}」裡明確綁定 CLI。建議先去綁定要管理的 智能体 實例，再執行啟動、重啟或停止。'),
  guidanceSuggestionBind: _('1. 请到「{settings}」里确认当前绑定的 CLI 与 智能体 安装路径是否指向你想管理的实例。', '1. Open "{settings}" and confirm the currently bound CLI and 智能体 install path point to the instance you want to manage.', '1. 請到「{settings}」裡確認目前綁定的 CLI 與 智能体 安裝路徑是否指向你想管理的實例。'),
  ...
  guidanceSuggestionInstallations: _('3. 下面列出了当前检测到的 智能体 安装，通常可以据此判断应该绑定哪一个 CLI。', '3. The detected 智能体 installations are listed below. They usually help you decide which CLI should be bound.', '3. 下方列出了目前檢測到的 智能体 安裝，通常可據此判斷應該綁定哪一個 CLI。'),
  ...
  guidanceInstallationsTitle: _('已检测到的 智能体 安装', 'Detected 智能体 installations', '已檢測到的 智能体 安裝'),
  ...
  guidanceWhyMultiInstall: _('你的电脑上安装了多个 智能体，面板需要知道应该管理哪一个，以免操作到错误的实例。', 'Multiple 智能体 installations exist on your computer. The panel needs to know which one to manage so it doesn\'t accidentally control the wrong instance.', '你的電腦上安裝了多個 智能体，面板需要知道應該管理哪一個，以免操作到錯誤的實例。'),
  ...
  guidanceWhyForeignUnbound: _('端口上已有 Gateway 在运行，但你还没告诉面板应该管理哪个 智能体。请先绑定再操作。', 'A Gateway is already running on the port, but you haven\'t told the panel which 智能体 to manage. Please bind first.', '連接埠上已有 Gateway 在運行，但你還沒告訴面板應該管理哪個 智能体。請先綁定再操作。'),
  ...
  dockerManagerHint: _('管理当前 Web 后端可访问的 Docker 节点与 智能体 容器。适合本机 socket，也支持填写远程 Docker API 节点。', 'Manage Docker nodes and 智能体 containers reachable from the current Web backend. Works with the local socket and remote Docker API endpoints.', '管理目前 Web 後端可訪問的 Docker 節點與 智能体 容器。適用於本機 socket，也支援遠端 Docker API 節點。'),
  ...
  dockerNoContainers: _('暂无 智能体 容器', 'No 智能体 containers yet', '暫無 智能体 容器'),
  ...
  dockerCreateTitle: _('创建 智能体 容器', 'Create 智能体 Container', '建立 智能体 容器'),
  ...
  upgradeConfirm: _('确定要将 智能体 切换到当前面板推荐的稳定{source}{version}吗？\n切换过程中 Gateway 会短暂中断。\n如果你想尝试最新版，请到「关于」页手动切换版本并自测兼容性。', 'Switch 智能体 to the panel-recommended stable {source}{version}?\nGateway will be briefly interrupted.\nTo try the latest version, manually switch in the About page.', '確定要將 智能体 切換到目前面板推薦的穩定{source}{version}吗？\n切換過程中 Gateway 會短暂中斷。\n如果你想尝試最新版，請到「關於」頁手動切換版本並自測相容性。'),
  ...
  upgradeScene: _('升级 智能体', 'Upgrade 智能体', '升級 智能体'),
  ...
  cleanupNoInstalls: _('未检测到 智能体 安装', 'No 智能体 installations detected', '未檢測到 智能体 安裝'),
  cleanupMultiSummary: _('检测到 {count} 个 智能体 安装，重复安装是 99% 问题的根源。建议只保留一个，卸载其余副本。', '{count} 智能体 installations detected. Duplicate installs cause 99% of issues. Keep one and uninstall the rest.', '檢測到 {count} 個 智能体 安裝，重複安裝是 99% 問題的根源。建議只保留一個，卸載其餘副本。'),
  cleanupSingleSummary: _('只检测到 1 个 智能体 安装，状态正常。', 'Only 1 智能体 installation detected. Status is normal.', '只檢測到 1 個 智能体 安裝，狀態正常。'),
  ...
  cleanupDangerDesc: _('以下操作将卸载所有 智能体 安装（包括当前使用的）。通常不需要这样做，除非你想彻底清理后重新安装。', 'The following will uninstall ALL 智能体 installations (including the active one). This is usually unnecessary unless you want to start completely fresh.', '以下操作將卸載所有 智能体 安裝（包括目前使用的）。通常不需要這樣做，除非你想徹底清理後重新安裝。'),
  ...
  cleanupConfirmUninstall: _('确定要卸载所有 智能体 安装吗？\n配置文件会被保留，可以重新安装后恢复。', 'Uninstall all 智能体 installations?\nConfig files will be preserved and can be restored after reinstalling.', '確定要卸載所有 智能体 安裝嗎？\n設定檔案會被保留，可以重新安裝後恢復。'),
  cleanupConfirmUninstallConfig: _('确定要卸载所有 智能体 并删除配置目录吗？\n⚠️ 这将删除所有配置、Agent 数据和会话记录，无法恢复！', 'Uninstall all 智能体 AND delete config directory?\n⚠️ This will delete all config, agent data, and session history permanently!', '確定要卸載所有 智能体 並刪除設定目錄嗎？\n⚠️ 這將刪除所有設定、Agent 資料和會話記錄，無法恢復！'),
