...
export default {
  ...
  openclawDir: _('智能体 配置目录', '智能体 Config Directory', '智能体 設定目錄', '智能体 設定ディレクトリ', '智能体 설정 디렉터리'),
  openclawSearchPaths: _('额外 智能体 搜索路径', 'Extra 智能体 Search Paths', '額外 智能体 搜尋路徑', '追加の 智能体 検索パス', '추가 智能体 검색 경로'),
  dockerDefaults: _('Docker 默认配置', 'Docker Defaults', 'Docker 預設配置', 'Docker デフォルト設定', 'Docker 기본 설정'),
  openclawCli: _('智能体 CLI 绑定', '智能体 CLI Binding', '智能体 CLI 綁定', '智能体 CLI バインド', '智能体 CLI 바인딩'),
  ...
  cliBindHint: _('选择面板实际使用的 智能体 CLI，适用于多版本共存场景', 'Select which 智能体 CLI the panel should use, useful when multiple versions coexist', '選擇面板實際使用的 智能体 CLI，適用於多版本共存場景', 'パネルが使用する 智能体 CLI を選択、複数バージョン共存時に便利', '패널이 사용할 智能体 CLI 선택, 여러 버전 공존 시 유용'),
  ...
  searchPathsPlaceholder: _('每行填写一个 CLI 文件或安装目录，例如 D:\\Tools\\智能体 或 E:\\AI\\openclaw.cmd', 'One CLI file or install directory per line, e.g. D:\\Tools\\智能体 or E:\\AI\\openclaw.cmd', '每行填寫一個 CLI 檔案或安裝目錄，例如 D:\\Tools\\智能体 或 E:\\AI\\openclaw.cmd', 'CLI ファイルまたはインストールディレクトリを 1 行ずつ入力してください。例: D:\\Tools\\智能体 または E:\\AI\\openclaw.cmd'),
  ...
  dockerDefaultImage: _('默认 智能体 镜像', 'Default 智能体 Image', '預設 智能体 映像', 'デフォルト 智能体 イメージ', '기본 智能体 이미지'),
  ...
  dirHint: _('自定义 智能体 配置目录路径。修改后需要重启面板生效。目标目录必须存在且包含 openclaw.json。', 'Custom 智能体 config directory path. Restart required after changes. Target directory must exist and contain openclaw.json.', '自定義 智能体 設定目錄路徑。修改后需要重啟面板生效。目標目錄必須存在且包含 openclaw.json。', 'カスタム 智能体 設定ディレクトリパス。変更後は再起動が必要です。対象ディレクトリは存在し、openclaw.json を含む必要があります。'),
