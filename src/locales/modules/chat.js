...
export default {
  ...
  responseTimeout: _('等待回复超时（{seconds}秒无响应），请检查 智能体 是否正常运行', 'Response timed out ({seconds}s no response). Please check if 智能体 is running.', '等待回覆逾時（{seconds}秒無回應），請檢查 智能体 是否正常運行'),
  ...
  guideDesc: _('此页面通过 <b>Gateway</b> 连接 智能体 的 AI Agent，对话由你部署的 智能体 服务处理。', 'This page connects to your 智能体 AI Agent via <b>Gateway</b>. Conversations are processed by your deployed 智能体 service.', '此頁面通過 <b>Gateway</b> 連線 智能体 的 AI Agent，對話由你部署的 智能体 服務處理。'),
  guideHint: _('如需使用 ClawPanel 内置 AI 助手（独立于 智能体），请前往左侧菜单「AI 助手」页面。', 'To use ClawPanel\'s built-in AI Assistant (independent of 智能体), go to "AI Assistant" in the left menu.', '如需使用 ClawPanel 內置 AI 助手（獨立於 智能体），請前往左側菜單「AI 助手」頁面。'),
  ...
  hostedHint: _('托管 Agent 会持续引导 智能体 完成此目标。模型使用 <a href="#/assistant" class="hosted-agent-link">AI 助手</a> 的配置。', 'Hosted Agent will continuously guide 智能体 to complete this goal. Uses <a href="#/assistant" class="hosted-agent-link">AI Assistant</a> model config.', '托管 Agent 會持續引导 智能体 完成此目標。模型使用 <a href="#/assistant" class="hosted-agent-link">AI 助手</a> 的設定。'),
  ...
  hostedAutoStopSignal: _('智能体 回复包含完成信号，自动停止', '智能体 reply contains completion signal, auto-stopping', '智能体 回覆包含完成信號，自動停止'),
  ...
  compactionUnsupported: _('当前 Gateway 版本不支持压缩历史功能，请升级到 智能体 2026.4.9+', 'Compaction history requires 智能体 2026.4.9+', '目前 Gateway 版本不支援壓縮歷史功能，請升級到 智能体 2026.4.9+'),
}
