...
/** 递归展平嵌套的 locale 模块，支持扁平与嵌套两种结构 */
function _flattenEntries(obj, lang) {
  const result = {}
  for (const [key, value] of Object.entries(obj)) {
    if (value && typeof value === 'object') {
      const firstKey = Object.keys(value)[0]
      // 判断是否为 _() 生成的翻译对象：首 key 是受支持的语言代码
      if (typeof value[firstKey] === 'string' && SUPPORTED_LANGS.includes(firstKey)) {
        result[key] = value[lang] || value['zh-CN'] || key
      } else {
        // 嵌套分区（如 guardian/gateway/ws），递归展平
        result[key] = _flattenEntries(value, lang)
      }
    }
  }
  return result
...
export function buildLocales() {
  ...
  for (const lang of SUPPORTED_LANGS) {
    ...
    for (const [mod, entries] of Object.entries(MODULES)) {
      result[lang][mod] = _flattenEntries(entries, lang)
    }
  }
  return result
}
