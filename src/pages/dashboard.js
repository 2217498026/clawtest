...
import { showLoadingOverlay } from '../components/modal.js'
...
export async function render() {
  ...
  page.innerHTML = `
    ...
    <div class="quick-actions">
      ...
      <button class="btn btn-secondary" id="btn-check-update" style="visibility: hidden;">${t('dashboard.checkUpdate')}</button>
      <button class="btn btn-secondary" id="btn-create-backup"  style="visibility: hidden;">${t('dashboard.createBackup')}</button>
    </div>
...
/*   const btnCreateBackup = page.querySelector('#btn-create-backup') */
  ...
  page.addEventListener('click', async (e) => {
    ...
    if (action === 'start-gw') {
      const loading = showLoadingOverlay(t('dashboard.startingGateway'))
      try {
        await api.startService('ai.openclaw.gateway')
        toast(t('dashboard.gwStartSent'), 'success')
        setTimeout(() => loadDashboardData(page), 100)
      } catch (err) {
    ...
    if (action === 'stop-gw') {
      const loading = showLoadingOverlay(t('dashboard.stoppingGateway'))
      try {
    ...
    if (action === 'restart-gw') {
      const loading = showLoadingOverlay(t('dashboard.restartingGateway'))
      try {
  ...
  btnRestart?.addEventListener('click', async () => {
    ...
    while (Date.now() - t0 < 30000) {
      ...
      loading.setText(t('dashboard.restartingGateway') + ` (${sec}s)`)
