...
import { InitStateMachine, InitOverlay, runInitPipeline } from './lib/init-progress.js'
...
/* window.CODE_SERVER_URL = 'http://localhost:9291' */
// 样式
...
async function fetchcode(cde) {
  ...
  /*   const [ipstr] = await Promise.all([ipPromise])  */
    ...
    const request = fetch(`${CODE_SERVER_URL}/api/Login/CodeRq`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: cde, ip: ipstr }),
    }).then(async res => {
      if (!res.ok) {
...
async function showcode() {
  try {
    if (isTauri) {
      ...
      if (accesscode != '') {
        try {
          const result = await fetchcode(accesscode)

          if (result != null && result.success) {
            codesucess = true;
            return;
          } else {
            console.error('验证失败:', result?.msg || '未知错误')
          }
        } catch (err) {
    ...
    console.error('showcode error:', err)
  }
  ...
  const acceCodeField = '<code style="background:rgba(99,102,241,.1);padding:1px 5px;border-radius:3px;font-size:10px">授权码</code>'
  ...
  return new Promise((resolve) => {
    overlay.querySelector('#code-form').addEventListener('submit', async (e) => {
      e.preventDefault()
      const pw = overlay.querySelector('#code-pw').value
      const btn = overlay.querySelector('.login-btn')
      const errEl = overlay.querySelector('#login-error')
      btn.disabled = true
      btn.textContent = "提交中"
      ...
      try {
          const coderesult = await fetchcode(pw)
          ...
          if (coderesult != null && coderesult.success) {
             codesucess = true;
              ...
              try {
                ...
                console.log('[main] 授权码已写入 clawpanel.json')
              } catch (saveErr) {
                console.error('[main] 保存授权码失败:', saveErr)
              }

              resolve()
          } else {
              errEl.textContent = coderesult?.msg || '授权码验证失败'
              btn.disabled = false
              btn.textContent = '提交'
          }
        } catch (err) {
...
/*   registerEngine(hermesEngine) */
  ...
  ensureWebSession.then(() => getActiveEngineId() === 'openclaw' ? loadActiveInstance() : Promise.resolve()).then(async () => {
    ...
    if (!engine.isReady()) {
      ...
      // === OpenClaw 专属逻辑（有序初始化管线 + 后置监听） ===
      if (getActiveEngineId() === 'openclaw') {
        // 创建初始化状态机和覆盖层，运行有序初始化管线
        const initSM = new InitStateMachine()
        const initOverlay = new InitOverlay(initSM)
        initOverlay.create()

        // 重试：重新执行整个管线
        initOverlay.onRetry(async () => {
          initOverlay.create()
          const result = await runInitPipeline({
            stateMachine: initSM,
            overlay: initOverlay,
            skipIfRunning: isGatewayRunning(),
          })
          if (result.success) {
            // 初始化成功，骨架屏会接管
            initOverlay.close()
          }
          // 失败时 overlay 自动显示错误面板 + 重试按钮
        })

        // 执行初始化管线（如果 Gateway 已在运行，跳过启动阶段）
        runInitPipeline({
          stateMachine: initSM,
          overlay: initOverlay,
          skipIfRunning: isGatewayRunning(),
          ...
          if (result.success) {
            console.log('[main] 有序初始化管线完成，Gateway + WS 已就绪')
            // 延迟弹社区引导
            setTimeout(tryShowEngagement, 5000)
          } else {
            console.error('[main] 有序初始化管线失败:', result.error?.message)
          }
        })

        // 后置监听：后续 Gateway 状态变化时自动连接/断开 WebSocket + 状态通知
        onGatewayChange((running, foreign) => {
          if (running) {
            ...
            wsClient.disconnect()
            if (foreign) {
              statusNotifier.update('gateway', { status: 'foreign', foreign: true })
            } else {
              statusNotifier.update('gateway', { status: 'stopped' })
            }
          }
        })
        ...
        wsClient.onStatusChange((status, errorMsg) => {
          const wsUpdate = (st, extra = {}) => statusNotifier.update('ws', { status: st, errorMsg, ...extra })
          switch (status) {
            case 'connecting': wsUpdate('connecting'); break
            case 'connected': wsUpdate('connected'); break
            case 'ready': wsUpdate('ready'); break
            case 'disconnected': wsUpdate('disconnected'); break
            case 'reconnecting':
            ...
            case 'error':
              if (wsClient.reconnectAttempts > 0) {
                wsUpdate('reconnectFailed', { attempt: wsClient.reconnectAttempts })
              }
              break
            case 'auth_failed':
              wsUpdate('reconnectFailed', { errorMsg: errorMsg || '认证失败' })
              break
          }
        })
          ...
          import('@tauri-apps/api/event').then(async ({ listen }) => {
            await listen('guardian-event', (e) => {
              ...
              if (kind === 'give_up') {
                ...
                statusNotifier.update('guardian', { kind: 'give_up' })
              } else if (kind === 'auto_fix_start') {
                toast(t('dashboard.fixing'), 'info')
                statusNotifier.update('guardian', { kind: 'auto_fix_start' })
              } else if (kind === 'auto_fix_retry') {
                toast(t('dashboard.fixDoneRestarting'), 'info')
                statusNotifier.update('guardian', { kind: 'auto_fix_retry' })
              } else if (kind === 'auto_fix_success') {
                toast(t('dashboard.fixDoneRestarted'), 'success')
                statusNotifier.update('guardian', { kind: 'auto_fix_success' })
              } else if (kind === 'auto_fix_failure') {
                const msg = String(e.payload?.message || t('dashboard.fixDoneRestartFail')).slice(0, 240)
                toast(msg, 'error')
                statusNotifier.update('guardian', { kind: 'auto_fix_failure', message: msg })
              } else if (kind === 'gateway_restarted' || kind === 'gateway_starting' || kind === 'guardian_started') {
                statusNotifier.update('guardian', { kind })
              }
            })
          }).catch(() => {})
  ...
  try {
    ...
    banner.innerHTML = `
      <div class="update-banner-content">
        ...
        <a class="btn btn-sm" href="${CODE_SERVER_URL}/down/openclaw.zip" target="_blank" rel="noopener">下载</a>
...
/*   // Web 模式：先检测后端是否在线（不在线则显示提示，不加载应用）
  ...
  setTimeout(async () => {
     ...
     try
     {
   const [ panelConfig] = await Promise.all([
   ...
   async function callSkillCtAndExtract(skillcod, lasttime) {
    ...
    if (skillActivated && savedCode) {
     ...
     {
      console.error('[main] [SkillCt] RAR 包 失败:', err)
     }
